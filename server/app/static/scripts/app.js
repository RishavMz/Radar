...
// Signal quality breakpoints (%) — mirror of server-side values, used only for CSS class
...
function sparkline(values, { color = COLOR_RSSI, zeroLine = false } = {}) {
  ...
  <svg viewBox="0 0 ${SPARK_W} ${SPARK_H}" preserveAspectRatio="none" class="sparkline-svg">
    ${zeroEl}
    <path d="${path}" fill="none" stroke="${color}" stroke-width="${SPARK_STROKE_WIDTH}"
...
function cardHTML(d) {
  ...
  const speedColor = movCls === 'closer' ? COLOR_CLOSER
                   : movCls === 'away'   ? COLOR_AWAY
                   : COLOR_STABLE;

  const rssiGraph  = sparkline(d.rssi_history  || [], { color: COLOR_RSSI });
  const speedGraph = sparkline(d.speed_history || [], { color: speedColor, zeroLine: true });
